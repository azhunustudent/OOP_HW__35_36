...
public:
    ...
    // перегрузить оператор + (оператор должен новый список, 
    // содержащего элементы обоих списков для которых вызывался оператор);
    TList<T> operator+ (const TList<T>& obj) const
    {
        TList<T> result(*this);
